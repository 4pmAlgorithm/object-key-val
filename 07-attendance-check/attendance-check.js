...
function attendanceCheck(day){
  let result = []
  classRoom.forEach(eachStudentObj => {
   Object.values(eachStudentObj).forEach(eachDay => {
      eachDay.forEach(eachEachDay => {
        if(Object.keys(eachEachDay)[0] === day){
        if(Object.values(eachEachDay)[0] === true ){
          result.push(Object.keys(eachStudentObj)[0])
        }}
      })
    })
  })
